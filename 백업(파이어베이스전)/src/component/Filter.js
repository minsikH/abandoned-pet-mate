...
const Filter = ({ query, onChange }) => {
  ...
  useEffect(() => {
    setUpdatedQuery(query); // query prop이 변경될 때 updatedQuery 상태를 업데이트
  }, [query]);
  ...
  const handleUpKindChange = (value) => {
    ...
    setupKind(value);
    /*     setUpdatedQuery({ ...query, UPKIND_NM: upKindCode }); // onChange를 호출하지 않고 업데이트된 쿼리를 설정 */
    setUpdatedQuery({ ...query, UPKIND_NM: upKindCode, KIND_NM: "" }); // 품종 초기화
    /*     onChange({ ...query, UPKIND_NM: upKindCode, KIND_NM: "" }); // 쿼리 변경 및 조회 */
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onChange(updatedQuery); // "조회" 버튼을 클릭했을 때 업데이트된 쿼리로 onChange를 호출
  };
